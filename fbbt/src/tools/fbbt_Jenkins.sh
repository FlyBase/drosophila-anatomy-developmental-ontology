...
echo ''
cat tmp6.obo | sed 's/^xref: OBO_REL:part_of/xref_analog: OBO_REL:part_of/' | sed 's/^xref: OBO_REL:has_part/xref_analog: OBO_REL:has_part/' | egrep -v "^property_value: .+$" | egrep -v "^owl-axioms: .+$"  > oort/fly_anatomy.obo  # Perhaps just make a generic substitution for xref: OBO_REL ?

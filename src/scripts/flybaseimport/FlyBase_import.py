...
def FBgn_template_from_ID_list(ID_list='../../../ontology/seed.txt', outfile='./template.tsv'):
    # create list of FBgns from ID_list (input file with all IDs used in ontology)
    ...
    for gene in FBgn_list:
        ...
        longname = list(set([n for n in FBgn_dataframe.query(
            'fbid == @gene & is_current & stype == "fullname"')['ascii_name']]))
        symbol = list(set([n for n in FBgn_dataframe.query(
            'fbid == @gene & is_current & stype == "symbol"')['ascii_name']]))
        synonyms = list(set([n for n in FBgn_dataframe.query(
            'fbid == @gene')['ascii_name'] if n not in (longname + symbol)]))
        if len(longname) > 1:
    ...
    template.to_csv(outfile, sep="\t", header=True, index=False)


if __name__ == '__main__':
    FBgn_template_from_ID_list()
